  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-border p-6 shadow-soft max-w-md">
        <div className="w-16 h-16 rounded-full bg-orange-100 flex items-center justify-center text-orange-700 font-bold text-xl mb-4">{session?.user?.name?.charAt(0) || "P"}</div>
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3"><Shield className="w-4 h-4 text-text-muted" /><div><p className="text-xs text-text-muted">Role</p><p className="font-medium text-text capitalize">{(session?.user as Record<string, string>)?.role || "pharmacy"}</p></div></div>
        </div>
